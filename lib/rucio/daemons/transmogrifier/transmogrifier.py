...
class Transmogrifier(Daemon):
    def __init__(self, chunk_size: int = 100, **_kwargs) -> None:
        """
        :param chunk_size: Number of requests per cycle
        """
        super().__init__(daemon_name="transmogrifier", **_kwargs)
    ...
    def _run_once(self, heartbeat_handler: "HeartbeatHandler", **_kwargs) -> tuple[bool, Any]:
        ...
        subscriptions = Transmogrifier._get_subscriptions(logger=logger)
        ...
        for did in list_new_dids(
            ...
            for subscription in subscriptions:
                ...
                if Transmogrifier.__is_matching_subscription(subscription, did) is True:
                    ...
                    for cnt, rule_dict in enumerate(rules):
                        ...
                        rule_dict = Transmogrifier.__get_rule_dict(rule_dict, subscription)
                        ...
                        if chained_idx:
                            ...
                            selected_rses = Transmogrifier._select_algorithm(
                                algorithm,
                            ...
                            (
                                ...
                                create_rule,
                                wont_reevaluate,
                            ) = Transmogrifier.__split_rule_select_rses(
    ...
    @staticmethod
    def _get_subscriptions(logger: Callable = logging.log) -> list[dict]:
        """
        Extract the list of active subscriptions and exclude the ones that have bad RSE expression.
        :param logger: Logger to use to log messages.
    ...
    @staticmethod
    def __is_matching_subscription(subscription: dict, did: dict) -> bool:
        """
        ...
        :return: True if DID matches subscription, False otherwise.
        """
    ...
    @staticmethod
    def _select_algorithm(algorithm: str, rule_ids: list, params: dict, logger: "Callable") -> dict:
        """
        Method used in case of chained subscriptions

        :param algorithm: Algorithm used for the chained rule. Now only associated_site
                        associated_site : Choose an associated endpoint according to the RSE attribute associated_site
